...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
   ...
   NSString *str = [self createPhoneNumber:@[@1, @2, @3, @4, @5, @6, @7, @8, @9, @0] type:1];
    ...
    str = [self _createPhoneNumber:@[@"1", @"2", @"3", @"4", @"5", @"6", @"7", @"8", @"9", @"0"]  inputType:@"1"];
    NSLog(@"str is %@",str);
...
- (NSString *)createPhoneNumber:(NSArray *)numbers type:(NSInteger)type{
    
    NSString *sub1 =  [[numbers subarrayWithRange:NSMakeRange(0, 3)] componentsJoinedByString:@""];
    NSString *sub2 =  [[numbers subarrayWithRange:NSMakeRange(3, 3)] componentsJoinedByString:@""];
    NSString *sub3 = [[numbers subarrayWithRange:NSMakeRange(6, numbers.count - 6)] componentsJoinedByString:@""];
    switch (type) {
        case 1:
            return [[[[[[@"(" stringByAppendingString:sub1]
                        stringByAppendingString:@")"]
                       stringByAppendingString:@" "]
                      stringByAppendingString:sub2]
                     stringByAppendingString:@"-"]
                    stringByAppendingString:sub3];
        case 2 :
            return [[[[[@"+" stringByAppendingString:sub1]stringByAppendingString:@"-"]stringByAppendingString:sub2]stringByAppendingString:@"-"]stringByAppendingString:sub3];
        case 3:
            return [[[[sub1 stringByAppendingString:@"-"] stringByAppendingString:sub2] stringByAppendingString:@" "] stringByAppendingString:sub3];
        default:
            return nil;
            break;
    }
...
- (NSString *)_createPhoneNumber:(NSArray *)numbers inputType:(NSString *)inputtype{
    NSDictionary *Maps = @{
                           @"1":@"(123) 456-7890",
                           @"2":@"+123-456-7890",
                           @"3":@"123-456 7890"
    ...
    NSInteger ct = 0;
    NSString *type = [Maps objectForKey:inputtype];
    NSMutableString *result = [NSMutableString string];
    NSInteger numberLoop = 0;
    while (ct < type.length) {
        NSString* c = [type substringWithRange:NSMakeRange(ct, 1)];
        if (!([c characterAtIndex:0] >='0' && [c characterAtIndex:0] <='9')) {
            [result appendFormat:c];
        }else{
            NSString *s = numbers[numberLoop];
            numberLoop+=1;
...
- (NSString *)createPhoneNumber:(NSArray *)numbers inputType:(NSString *)inputtype{
    //保存各个部分的区别
    NSDictionary *types = @{@"1":@[@"(",@") ",@"-"],
                            @"2":@[@"+",@"-",@"-"],
                            @"3":@[@"",@"-",@" "]
                            };
    NSArray *type =[types objectForKey:inputtype];
    //把数组分成三部分
    NSString *sub1 =  [[numbers subarrayWithRange:NSMakeRange(0, 3)] componentsJoinedByString:@""];
    NSString *sub2 =  [[numbers subarrayWithRange:NSMakeRange(3, 3)] componentsJoinedByString:@""];
    NSString *sub3 = [[numbers subarrayWithRange:NSMakeRange(6, numbers.count - 6)] componentsJoinedByString:@""];
    NSArray *subs = @[sub1,sub2,sub3];
    //拼接
    NSMutableString *string = [NSMutableString string];
 
    for ( int i = 0; i < types.count; i ++) {
         [string appendFormat:type[i]];
        [string appendFormat:subs[i]];
    }
    return string;
